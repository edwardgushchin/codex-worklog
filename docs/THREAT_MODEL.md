...
| A malicious path or planted worklog injects model instructions | Unsafe paths fail closed, model-visible values are sanitized and bounded, and automatic previous-session pointers come only from private plugin state. Worklog text is explicitly treated as untrusted history, never as instructions or user authorization. |
