...
6. Validate the plugin with the current Codex `plugin-creator` validator.
   ...
   after changing launcher/storage helpers; the repository validator checks them.
...
9. Retain a loaded hook command, remove/replace its entire temporary installed
   package, and invoke every lifecycle event. Require exit 0, a warning without
   blocking/continuation fields, and a safe diagnostic outside the cache. Test
   native reinstall via `scripts/update_plugin.py` in an isolated Codex home;
   check operation IDs and before/after cache versions. Direct failed `submit`
   must remain nonzero. Never use an enabled working task for this destructive
   cache-replacement test; legacy loaded hooks need disabling and a fresh task.
